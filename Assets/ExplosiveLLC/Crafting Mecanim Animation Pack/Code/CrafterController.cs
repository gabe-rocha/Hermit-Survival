...
namespace CraftingAnims {
	public class CrafterController : MonoBehaviour {
		...
		private void Update() {
			...
			// Facing switch.
			if(inputFacing) { isFacing = true; } else { isFacing = false; }

			// Slow time.
			if(Input.GetKeyDown(KeyCode.T)) {
				if(Time.timeScale != 1) { Time.timeScale = 1; } else { Time.timeScale = 0.15f; }
			}

			// Pause.
			if(Input.GetKeyDown(KeyCode.P)) {
				if(Time.timeScale != 1) { Time.timeScale = 1; } else { Time.timeScale = 0f; }
			}

			// Push-Pull
			if(charState != CrafterState.PushPull) { CameraRelativeInput(); } else { PushPull(); }
		...
		private float UpdateMovement() {
			...
			if(!isLocked &&
				...
				if(isRunning) { newVelocity = motion * runSpeed; } else if(isSpearfishing) { newVelocity = motion * spearfishingSpeed; } else { newVelocity = motion * walkSpeed; }
		...
		private void CheckForGrounded() {
			...
			if(Physics.Raycast((transform.position + offset), -Vector3.up, out RaycastHit hit, 100f)) {
				...
				if(distanceToGround < threshold) { isGrounded = true; } else { isGrounded = false; }
			}
		}
